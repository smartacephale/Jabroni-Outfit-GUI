<script setup lang="ts">
  ...
  const onInput = (e: Event, key: keyof typeof Duration) => {
...
<template>
  <div class=":uno: flex box-border border-2 p-.5 justify-center w-34 text-sm
      focus-within:border-gray-500 focus-within:ring-1 
      transition-all
      border-black
      bg-white dark:bg-gray-700">
    <template v-for="(config, key, index) in Duration" :key="key">
      <span v-if="index > 0" class=":uno: mx-0.5 text-slate-400 font-bold select-none">:</span>

      <input :ref="(el) => config.inputRef.value = el as HTMLInputElement" v-model="config.value.value" type="text"
        placeholder="00"
        class=":uno: dark:text-slate-300 w-7 text-center focus:outline-none focus:bg-transparent dark:bg-transparent"
        @input="onInput($event, key)" @keydown="onKeyDown($event, key)" @blur="onBlur(key)">
    </template>
  </div>
</template>
